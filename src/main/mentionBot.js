...
async function checkTodoCompletion({ channelId, msgText, log, postTodoListNow, doorayClient, trigger }) {
  ...
  // 이 채팅방이 "메일함 [요청]도 투두리스트에 자동 포함" 설정이면, 완료/삭제 등을 판단하기
  // 전에 최신 메일 요청부터 투두 카드로 먼저 반영합니다. 이 동기화는 원래 목록을 다시 올릴
  // 때(postTodoListNow)만 돌았는데, 그 사이에 새로 생긴 메일 요청은 아직 카드가 없어서
  // 채팅으로 "완료"라고 해도 AI가 찾을 카드 자체가 없어 무시되는 문제가 있었습니다.
  try {
    const cfg = loadConfig()
    if ((cfg.todoMailSyncChannels || []).includes(channelId)) {
      const { syncMailRequestsToTodo } = require('./index')
  ...
  const prompt = [
    `오늘 날짜는 ${todayIso}입니다.`,
    '',
    '아래는 어느 채팅방의 "오늘의 할 일" 목록, 등록된 태그 목록, 방금 올라온 메시지',
    '1건입니다. 메시지를 보고 아래 항목들을 판단해 각 마커 사이에 적으세요. 모든 항목은',
    '[MARKER]내용[/MARKER] 형태로 출력하고, 해당 없거나 확신이 없으면 절대 추측하지 말고',
    '[MARKER][/MARKER]처럼 비워서 그대로 출력하세요.',
    '',
    '[QUERY_DATE] 완료/추가/태그변경/삭제/수정 지시가 조금이라도 섞이지 않고, 그냥 "오늘/',
    '내일/특정 날짜에 할 일 뭐야?"처럼 목록 자체만 물어보는 순수 조회 질문이면, 그 날짜를',
    `오늘(${todayIso}) 기준으로 계산해 TODAY 또는 YYYY-MM-DD 형식으로 적으세요. 이 경우`,
    '아래 나머지 마커는 전부 빈 채로 출력하면 됩니다. 순수 조회가 아니면 비우고 아래를',
    '평소대로 판단하세요.',
    '',
    '[DONE_IDS] 목록 중 하나 이상을 이미 끝냈다는 보고면, 완료된 항목의 [ ] 안 ID를 쉼표로',
    '구분해 적으세요. 쉼표 등 구분자 없이 "A 완료 B 완료"처럼 줄바꿈이나 띄어쓰기로만 여러',
    '건이 이어져 있어도, 언급된 완료 건을 빠짐없이 전부 찾아 넣으세요.',
    '',
    '[NEW_ITEMS] "~추가", "~등록해줘" 같은 명시적 지시어가 있거나, 지시어 없이도 "7/29 메타',
    '소재 종료 예약"처럼 (날짜)+(할 일 이름)만으로 명백히 새 업무면 새 할 일입니다. 질문/',
    '잡담/의견처럼 등록 의도가 없으면 넣지 마세요 — 단, "OO 태그로 할 일 생성"처럼 1. 2. 3.',
    '번호를 매겨 명시적으로 등록을 지시한 목록은 예외로, 개별 항목이 사소하거나 농담 같아',
    '보여도 번호 매긴 항목 수와 등록 항목 수가 같도록 전부 등록하세요(일부만 고르면 안 됨).',
    '항목마다 한 줄에 "YYYY-MM-DD|태그ID|매체명|할 일 내용" 형식으로 적으세요.',
    `- 날짜: 언급되어 있으면 오늘(${todayIso}) 기준으로 계산한 날짜, 없으면 오늘 날짜`,
    '  그대로. "추가/등록" 같은 지시어와 날짜 표현은 할 일 내용에서 빼세요.',
    '- 태그ID: "재우 태그로"처럼 사람 이름/업무 성격을 태그처럼 말했으면 아래 태그 목록에서',
    '  정확히 일치하는 것만 찾아 ID를 적으세요. 없거나 목록에 없으면 비워두고 절대 새로',
    '  지어내지 마세요(태그 이름 자체도 할 일 내용에 다시 넣지 마세요).',
    '- 매체명: "메타/구글/카카오/네이버/SA" 같은 매체·채널이 있으면 적으세요. 아래 매체',
    '  목록에 같은 매체가 있으면(표기가 달라도 같은 매체면, 예: "메타"="Meta") 그 표기를',
    '  그대로 쓰고, 없는 새 매체면 새로 지어써도 됩니다(태그와 달리 매체는 자동 생성 가능).',
    '  모르면 비워두세요.',
    '- 오늘 할 일인지 특정 날짜 예약인지 스스로 확신이 안 서면(예: "다음 주 중으로",',
    '  "조만간") 여기 넣지 말고 [AMBIGUOUS]로 보내세요.',
    '',
    '[AMBIGUOUS] 새 할 일인 건 맞는데 오늘 할 일인지 특정 날짜 예약인지 확신할 수 없으면,',
    '"할 일 내용|되물을 질문" 형식으로 딱 한 건만 적으세요(되물을 질문은 채팅방에 그대로',
    '보낼 것이니 짧고 자연스러운 존댓말로). 여러 건이면 확실한 건 NEW_ITEMS로, 애매한 건만',
    '여기로 나눠 적으세요.',
    '',
    '[TAG_CHANGES] "이거 재우 태그로 바꿔줘"처럼 목록 항목을 아래 태그 목록 중 하나로',
    '옮겨달라는 명확한 요청이면 "카드ID|태그ID" 형식으로 한 줄씩 적으세요. "미분류 전부"/',
    '"~에 있는거 다"처럼 특정 항목을 콕 집지 않고 조건으로 뭉뚱그려 말했으면, 그 조건과',
    '일치하는 항목을 목록에서 전부 찾아 한 건도 빠짐없이 적으세요.',
    '',
    '[NEW_TAG_REQUEST] TAG_CHANGES로 옮기려는 태그 이름이 아래 태그 목록에 없는 새',
    '태그면, 절대 지어내서 만들지 말고 대신 "새태그이름|카드ID1,카드ID2" 형식으로 옮기려던',
    '항목 ID와 함께 딱 한 건만 적으세요(실제 생성은 사람 확인 후 진행하므로 여기선 요청만',
    '표시).',
    '',
    '[DELETE_IDS] "삭제해줘", "지워줘", "취소해줘"처럼 완료가 아니라 아예 목록에서',
    '없애달라는 요청이면 해당 항목의 [ ] 안 ID를 쉼표로 구분해 적으세요. "끝냈어", "했어"',
    '같은 완료 보고와 절대 헷갈리지 마세요(그건 DONE_IDS입니다).',
    '',
    '[EDIT_ITEMS] "아까 그거 OO로 바꿔줘", "문구 좀 고쳐줘"처럼 태그/매체가 아니라 할 일',
    '내용 문구 자체를 바꿔달라는 요청이면 "카드ID|새 문구" 형식으로 줄마다 적으세요. 카드가',
    '있는 곳(태그)은 그대로 두고 글자만 바꾸는 경우에만 씁니다 — TAG_CHANGES와 절대',
    '헷갈리지 마세요.',
    '',
    '[오늘의 할 일 목록]',
    listText,
...
function createMentionHandler({ doorayClient, doorayService, getConfig, getMyMemberId, log, postTodoListNow }) {
  return async function handleSocketMessage(data) {
    ...
    //
    // (2026-08-06) 예전에는 단어 검사만으로 여기 들어와서, "이 파일 드라이브에 올려줘" 같은
    // 요청까지 전부 업무 첨부로 새버렸습니다. 이제 단어 검사는 1차 후보 거르기로만 쓰고
    // 실제 판단은 클로드가 합니다. 'other'로 판단되면 여기 안 들어오고 아래 일반 흐름으로
    // 내려가, 클로드가 드라이브 업로드 등 알맞은 도구를 직접 고릅니다.
    if (isAttachFileCommand(question) &&
        (await judgeAttachIntent(question, { askClaude, cwd: workDir })) === 'task') {
